...
func (f formatter) formatFile(filePath string) ([]byte, []byte, error) {
	...
	for i, decl := range node.Decls {
		...
		stdlib := make([]importPathAndSpec, 0, len(genDecl.Specs))
		thirdParty := make([]importPathAndSpec, 0, len(genDecl.Specs))
		firstParty := make([]importPathAndSpec, 0, len(genDecl.Specs))
		...
		for _, imp := range stdlib {
			genDecl.Specs = append(genDecl.Specs, imp.spec)
		}
